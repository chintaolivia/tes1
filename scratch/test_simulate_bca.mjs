...
async function main() {
  ...
  const resTabs = await fetch("http://127.0.0.1:9996/json/list");
  ...
  // Trigger simulation payment click
  console.log("Triggering simulate payment click...");
  const simResult = await send("Runtime.evaluate", {
    expression:
      "handleSimulatePayment(document.getElementById('btn-simulate-pay'))",
    awaitPromise: true,
  });
  console.log("Simulate payment result:", simResult);
  ...
  fs.writeFileSync(
    path.join(artDir, "invoice_bca_paid_success.png"),
    Buffer.from(shot.data, "base64"),
  );
  console.log("Saved invoice_bca_paid_success.png");
